...
parseContent = function (request) {
    var requesturl = url.parse(request.url, true)
    var time = new Date(requesturl.query.iso)
 
    //Parse /api/parsetime?iso=2013-08-10T12:10:15.474Z
    if (requesturl.pathname == "/api/parsetime") {
        ...
        // Parse /api/unixtime?iso=2013-08-10T12:10:15.474Z
...
deliverResult = function (response, result /*, format*/ ) {
    /* var format = ("" || "jason") */
...
function startHttpServer( port ) {
    var server = http.createServer( function(req, res){
        ...
        var parsedJasonContent = parseContent(req)

        if (parsedJasonContent) {
            deliverResult(res, parsedJasonContent)
        } else {
            console.log("Paser: no result")
            res.writeHead(404)
            res.end()
        }
